...
async def yt2m_handler(bot: Client, m: Message):
    ...
    if not input or not hasattr(input, 'text'):
    ...
    try:
        ...
        # Sanitize the name to prevent filesystem issues
...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:
        input0 = await bot.listen(chat_id=m.chat.id, filters=filters.chat(editable.chat.id), timeout=20)
        if not input0 or not hasattr(input0, 'text'):
            raw_text = '1'
        else:
            raw_text = input0.text
        await input0.delete(True)
    ...
    try:
        input1 = await bot.listen(chat_id=m.chat.id, filters=filters.chat(editable.chat.id), timeout=20)
        if not input1 or not hasattr(input1, 'text'):
            raw_text0 = '/d'
        else:
    ...
    try:
        input2 = await bot.listen(chat_id=m.chat.id, filters=filters.chat(editable.chat.id), timeout=20)
        raw_text2 = input2.text
        await input2.delete(True)
    except asyncio.TimeoutError:
        raw_text2 = '1080'  # Default to high quality (1080p)
    ...
    try:
        input3 = await bot.listen(chat_id=m.chat.id, filters=filters.chat(editable.chat.id), timeout=20)
    ...
    try:
        input4 = await bot.listen(chat_id=m.chat.id, filters=filters.chat(editable.chat.id), timeout=20)
    ...
    try:
        input6 = await bot.listen(chat_id=m.chat.id, filters=filters.chat(editable.chat.id), timeout=20)
    ...
    try:
        input7 = await bot.listen(chat_id=m.chat.id, filters=filters.chat(editable.chat.id), timeout=20)
    ...
    if "/d" in raw_text7:
        ...
        channel_id = raw_text7.strip()
        # Ensure it's a valid integer
        try:
            channel_id = int(channel_id)
        except ValueError:
            await m.reply_text("**Invalid Channel ID format. Please provide a valid numeric ID or send /d**")
            return
    ...
    try:
        if raw_text == "1":
            # Validate channel access before sending message
            try:
    ...
    for i in range(arg-1, len(links)):
            ...
            try:
                ...
                # Generic classplusapp fallback (for other types)
                    ...
                    try:
                        ...
                        if response.status_code != 200 or "deployment" in response.text.lower() or "not found" in response.text.lower():
                            await m.reply_text(f"⚠️ **API Error for:** `{name1}`\n**Original URL:** `{url}`\n**API Response:** `{response.text[:200]}`\n\n**Note:** The API endpoint may be down. Contact admin.")
                            count += 1
                            failed_count += 1
                            continue
                        ...
                        await m.reply_text(f"⚠️ **API Call Failed for:** `{name1}`\n**Error:** `{str(api_error)}`\n**Original URL:** `{url}`")
                        count += 1
                        failed_count += 1
                        continue
                ...
                # Define quality and path for use in messages
                quality = f"{raw_text2}p"
                path = f"./downloads/{m.chat.id}"
                ...
                if "drive" in url:
                    ...
                    try:
                        ...
                        try:
                            ...
                            await m.reply_text(f"**Failed to send audio to channel. Error: {str(send_error)}**")
                            os.remove(f'{name}.{ext}')
                            continue
                        count += 1
                        os.remove(f'{name}.{ext}')
                    ...
                    try:
                        ...
                        await m.reply_text(f"**Failed to send message to channel. Error: {str(send_error)}**")
                        continue
                    ...
                    Show1 = f"<blockquote>🚀𝐏𝐫𝐨𝐠𝐫𝐞𝐬𝐬 » {progress:.2f}%</blockquote>\n┃\n" \
                           ...
                           f"╰━🖇️𝐑𝐞𝐦𝐚𝐢𝐧 » {remaining_links}\n" \
                           f"━━━━━━━━━━━━━━━━━━━━━━━━\n" \
                           f"<blockquote><b>⚡Dᴏᴡɴʟᴏᴀ𝐝ɪɴɢ Eɴᴄʀʏᴘᴛᴇᴅ Sᴛᴀʀᴛᴇᴅ...⏳</b></blockquote>\n┃\n" \
                           f'┣💃𝐂𝐫𝐞𝐝𝐢𝐭 » {CR}\n┃\n' \
                           f"╰━📚𝐁𝐚𝐭𝐜𝐡 » {b_name}\n" \
                           f"━━━━━━━━━━━━━━━━━━━━━━━━━\n" \
                           f"<blockquote>📚𝐓𝐢𝐭𝐥𝐞 » {name}</blockquote>\n┃\n" \
                ...
                try:
                    ...
                    await m.reply_text(f"**Failed to send error message to channel. Error: {str(send_error)}**")
                count += 1
                failed_count += 1
                continue
    ...
    try:
        if raw_text7 == "/d":
            ...
            try:
                ...
                await m.reply_text(f"**Failed to send completion message to channel. Error: {str(send_error)}**")
            await bot.send_message(m.chat.id, f"<blockquote><b>✅ Your Task is completed, please check your Set Channel📱</b></blockquote>")
    except Exception as e:
        await m.reply_text(f"**Failed to send completion message. Error: {str(e)}**")
...
async def reset_and_set_commands():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            # Reset
            async with session.post(url, json={"commands": []}) as resp:
                pass
            # Set new
